...
public sealed partial class ContainerLoadingApp
{
    bool BuildMobileShipmentHome()
    {
        ...
        var action = Horizontal(content, "ShipmentActions", 10); MobileButton(action, "+ TẠO CHUYẾN HÀNG", () => { BeginNewContainer(); RefreshMobileUI(); }, true);
    ...
    bool BuildMobileShipmentDetail()
    {
        ...
        SectionHeader(content, "KIỂM TRA CHUYẾN HÀNG", "Kiểm tra lỗi, cảnh báo và tiến độ của phương án hiện tại");
        var health = ShipmentIntelligence.Check(shipment);
        var healthCard = Vertical(content, "ShipmentHealth", health.errors.Count == 0 ? UiPrimarySoft : new Color(1f, .94f, .94f), 8, 20);
        MobileText(healthCard, health.StatusLabel, 28, health.errors.Count == 0 ? UiPrimary : UiDanger, FontStyle.Bold);
        MobileText(healthCard, health.errors.Count == 0 ? "Không có lỗi chặn trong phương án hiện tại." : $"Có {health.errors.Count} lỗi cần xử lý trước khi xuất báo cáo.", 22, UiMuted);
        SectionHeader(content, "DANH SÁCH CONTAINER", "Mỗi container có sơ đồ, mức sử dụng và kiểm tra riêng");
        ...
        MobileButton(content, "+ THÊM CONTAINER", AddShipmentContainer, true); var reports = Horizontal(content, "Reports"); MobileButton(reports, "BÁO CÁO QUẢN LÝ", ExportPdf, true); MobileButton(reports, "HƯỚNG DẪN ĐÓNG HÀNG", ExportOperationalPdf); MobileButton(content, "SAO LƯU", ExportPlanData);
        return true;
    }
