...
public sealed partial class ContainerLoadingApp : MonoBehaviour
{
    ...
    LoadingPlan plan; Shipment shipment; string shipmentPath; ScreenMode screen; Camera cam; Transform root;
    ...
    void Awake(){shipment=ShipmentPersistence.Load();if(shipment==null){plan=PlanPersistence.Load()??new LoadingPlan();shipment=ShipmentIntelligence.FromV2Plan(plan);}EnsureShipmentDefaults();plan=shipment.containers.FirstOrDefault()??CreateContainerForShipment();screen=ScreenMode.Home;SetupScene();Rebuild();SetupMobileUI();SetupUpdates();}
    void Update()
    {
    ...
    void EnsureShipmentDefaults(){ShipmentIntelligence.Normalize(shipment);foreach(var type in shipment.cargoTypes)if(type.color.a<=0)type.color=DefaultColor(type.id);}
    LoadingPlan CreateContainerForShipment(){var created=new LoadingPlan{name="Container 01",shipmentId=shipment.id,container=new ContainerConfig{name="40FT",length=12,width=3,height=3},cargoTypes=shipment.cargoTypes};shipment.containers.Add(created);return created;}
    ...
    void ReplaceCurrentContainer(LoadingPlan replacement){var index=shipment.containers.FindIndex(x=>x.id==plan.id);replacement.cargoTypes=shipment.cargoTypes;replacement.shipmentId=shipment.id;if(index>=0)shipment.containers[index]=replacement;plan=replacement;}
    void Undo(){if(undo.Count==0){status="Không còn thao tác để hoàn tác.";return;}redo.Push(JsonUtility.ToJson(plan));ReplaceCurrentContainer(JsonUtility.FromJson<LoadingPlan>(undo.Pop()));selectedId=null;status="Đã hoàn tác.";QueueAutosave();Rebuild(true);}
    void Redo(){if(redo.Count==0){status="Không còn thao tác để làm lại.";return;}undo.Push(JsonUtility.ToJson(plan));ReplaceCurrentContainer(JsonUtility.FromJson<LoadingPlan>(redo.Pop()));selectedId=null;status="Đã làm lại.";QueueAutosave();Rebuild(true);}
    ...
    IEnumerator AutosaveAfterDelay(){yield return new WaitForSecondsRealtime(.8f);autosaveRoutine=null;try{shipmentPath=ShipmentPersistence.Save(shipment,shipmentPath);}catch(Exception exception){Debug.LogException(exception);status="Không thể tự động lưu. Hãy nhấn Lưu sơ đồ.";if(mobileCanvas)RefreshMobileUI(true);}}
    ...
    void PreviewAutoFill(){if(plan.cargoTypes.Count==0)return;pendingAutoPlacementBackup=JsonUtility.ToJson(plan);var original=plan.cargoTypes.ToDictionary(x=>x.id,x=>x.quantity);foreach(var type in plan.cargoTypes)type.quantity=plan.placedCargo.Count(x=>x.cargoTypeId==type.id)+ShipmentIntelligence.Remaining(shipment,type);var added=PlanIntelligence.AutoFillRemaining(plan);foreach(var type in plan.cargoTypes)type.quantity=original[type.id];if(added==0){ReplaceCurrentContainer(JsonUtility.FromJson<LoadingPlan>(pendingAutoPlacementBackup));pendingAutoPlacementBackup=null;status="Không còn kiện hoặc vị trí phù hợp để xếp tự động.";RefreshMobileUI(true);return;}placementMode=false;selectedId=null;status=$"Đang xem trước {added} kiện được xếp tự động.";Rebuild(true);ShowAutoFillPreview(added);}
    ...
    void CancelAutoFill(){if(string.IsNullOrEmpty(pendingAutoPlacementBackup))return;ReplaceCurrentContainer(JsonUtility.FromJson<LoadingPlan>(pendingAutoPlacementBackup));pendingAutoPlacementBackup=null;status="Đã hủy bản xem trước xếp tự động.";Rebuild(true);}
    ...
    bool TrySaveCurrent(string previousPath=null)
    {
        ...
        busy=true;
        try{shipmentPath=ShipmentPersistence.Save(shipment,shipmentPath);status="Đã lưu dữ liệu an toàn.";return true;}
    ...
    void ExportPlanData()
    {
        if(busy)return;
        busy=true;
        try{lastDataExport=ShipmentPersistence.ExportJson(shipment);status=PdfPlatform.ShareJson(lastDataExport)?"Đã tạo bản sao dữ liệu JSON.":"Đã tạo JSON nhưng không mở được trình chia sẻ.";}
    ...
    {
        try { lastDataExport=ShipmentPersistence.ExportBackup();status=PdfPlatform.ShareJson(lastDataExport)?"Đã tạo bản sao lưu toàn bộ.":"Đã tạo bản sao lưu nhưng không mở được trình chia sẻ."; }
    ...
    void OpenPlan(string path){if(!PlanPersistence.TryLoad(path,out var loaded,out var error)){status="Không thể mở phương án: "+error;if(mobileCanvas)RefreshMobileUI();return;}shipment=ShipmentIntelligence.FromV2Plan(loaded);EnsureShipmentDefaults();plan=shipment.containers[0];shipmentPath=null;undo.Clear();redo.Clear();selectedId=null;placementMode=false;layer=0;status="Đã mở "+plan.name+".";screen=ScreenMode.Detail;Rebuild();ResetCamera();scroll=Vector2.zero;}
    ...
    void BeginNewContainer(){shipment=new Shipment();shipmentPath=null;plan=new LoadingPlan{shipmentId=shipment.id,cargoTypes=shipment.cargoTypes};shipment.containers.Clear();editingContainer=false;formDirty=false;containerCode="SHIP"+DateTime.Now.ToString("MMddHHmm");containerName="Container tùy chỉnh";containerLength="10";containerWidth="3";containerHeight="6";orderReference=customerName=containerNumber=sealNumber=destination=loadingDate=orderNotes="";status="Nhập thông tin chuyến hàng và container đầu tiên.";screen=ScreenMode.Container;scroll=Vector2.zero;}
    ...
    void SaveContainer()
    {
        ...
        if(editingContainer)
        {
            ...
            plan.name=code;plan.container.name=string.IsNullOrWhiteSpace(description)?"Container":description;plan.containerType=description;plan.container.length=l;plan.container.width=w;plan.container.height=h;plan.containerNumber=(containerNumber??"").Trim();plan.sealNumber=(sealNumber??"").Trim();plan.notes=notes;shipment.shipmentName=code;shipment.orderReference=reference;shipment.customerName=customer;shipment.destination=(destination??"").Trim();shipment.loadingDate=(loadingDate??"").Trim();shipment.notes=notes;status="Đã cập nhật chuyến hàng và container.";
        }
        else
        {
            plan=new LoadingPlan{name=code,shipmentId=shipment.id,containerType=description,containerNumber=(containerNumber??"").Trim(),sealNumber=(sealNumber??"").Trim(),container=new ContainerConfig{id=Guid.NewGuid().ToString("N"),name=string.IsNullOrWhiteSpace(description)?"Container":description,length=l,width=w,height=h},cargoTypes=shipment.cargoTypes};
            shipment.shipmentName=code;shipment.orderReference=reference;shipment.customerName=customer;shipment.destination=(destination??"").Trim();shipment.loadingDate=(loadingDate??"").Trim();shipment.notes=notes;shipment.containers.Add(plan);
    ...
    void SaveCargo()
    {
        ...
        var placedCount=shipment.containers.Sum(c=>c.placedCargo.Count(x=>x.cargoTypeId==type.id));if(quantity<placedCount){status=$"Không thể giảm số lượng xuống {quantity} vì toàn chuyến đã xếp {placedCount} kiện.";return;}
    ...
    void RequestDeleteCargo(CargoType type){if(shipment.containers.Any(c=>c.placedCargo.Exists(x=>x.cargoTypeId==type.id))){status="Không thể xóa vì loại hàng này đang được sử dụng trong một container.";return;}deleteCargoTarget=type;}
